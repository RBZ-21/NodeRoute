...
// ── Local derived types (not API types — computed client-side) ────────────────
...
function summarizeReceivables(invoices: FinancialInvoice[]): ReceivableRow[] {
  ...
  for (const invoice of invoices) {
    ...
    const existing = byCustomer.get(customerKey) || { customerKey, customerLabel, openBalance: 0, openInvoiceCount: 0, oldestIssueDate: '', oldestDueDate: '' };
...
export function FinancialsPage() {
  ...
  const summary = useMemo(() => {
    ...
    const monthRevenue  = invoices.filter((inv) => { const d = new Date(inv.created_at || ''); return d.getMonth() === month && d.getFullYear() === year; }).reduce((sum, inv) => sum + numberOr(inv.total), 0);
  ...
  function exportCsv() {
    if (!rangeRows.length) { alert('No rows to export for the selected date range.'); return; }
    const header   = ['Date', 'Sales', 'Invoices'];
    const csvRows  = rangeRows.map((row) => [row.date, row.sales.toFixed(2), String(row.invoiceCount)]);
    const csv      = [header, ...csvRows].map((r) => r.map((value) => `"${value.replace(/"/g, '""')}"`).join(',')).join('\n');
    const a        = document.createElement('a');
    const href     = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
    a.href = href; a.download = `daily-sales-${startDate || 'start'}-to-${endDate || 'end'}.csv`; a.click(); URL.revokeObjectURL(href);
  }
  ...
  const fetchError = invoicesQuery.error
    ? String((invoicesQuery.error as Error).message || 'Could not load financial data')
    : '';
        ...
        <CardContent>
          ...
          <div className="rounded-lg border border-border bg-card p-2">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Customer</TableHead><TableHead>Open Invoices</TableHead><TableHead>Open Balance</TableHead><TableHead>Oldest Invoice</TableHead><TableHead>Oldest Due</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {receivables.length ? (
                  ...
                  <TableRow><TableCell colSpan={5} className="text-muted-foreground">No unpaid invoices are open right now.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
          ...
          {showHistory ? (
            <div className="space-y-3 rounded-lg border border-border bg-muted/20 p-4">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Button className="lg:self-end" variant="secondary" onClick={exportCsv}>Export CSV</Button>
              </div>
              ...
              <div className="rounded-lg border border-border bg-card p-2">
                <Table>
                  <TableHeader>
                    <TableRow><TableHead>Date</TableHead><TableHead>Sales</TableHead><TableHead>Invoices</TableHead></TableRow>
                  </TableHeader>
                  <TableBody>
                    {rangeRows.length ? (
                      rangeRows.map((row) => (
                      ...
                      <TableRow><TableCell colSpan={3} className="text-muted-foreground">No sales rows in selected range.</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
