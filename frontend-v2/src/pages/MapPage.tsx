...
function driverName(d: DriverLocation) { return d.name || d.full_name || d.fullName || '—'; }
...
export function MapPage() {
  ...
  const plotMarkers = useCallback(
    (driverList: DriverLocation[], stopList: StopMarker[]) => {
      ...
      stopList.forEach((s) => {
        const lat = toLatLng(s.lat); const lng = toLatLng(s.lng);
        if (lat === null || lng === null) return;
        const normalized = String(s.status || '').toLowerCase().replace(/[\s_]+/g, '-');
        const color = stopColorHex[normalized] || '#6b7280';
        const marker = new gm.Marker({
          position: { lat, lng }, map: googleMapRef.current,
          icon: { path: gm.SymbolPath.CIRCLE, scale: 7, fillColor: color, fillOpacity: 0.9, strokeColor: '#ffffff', strokeWeight: 2 },
          title: s.address || `Stop ${s.stop_id || s.stopId || s.id || ''}`, zIndex: 1,
        });
        const info = new gm.InfoWindow({ content: `<div style="font-family:Inter,sans-serif;padding:4px;min-width:140px"><b>${s.address || 'Stop'}</b><br><span style="color:#666">Status: ${s.status || 'unknown'}</span>${s.driver ? `<br><span style="color:#666">Driver: ${s.driver}</span>` : ''}</div>` });
        marker.addListener('click', () => info.open(googleMapRef.current, marker));
        markersRef.current.push(marker);
      });
      driverList.forEach((d) => {
        const lat = toLatLng(d.lat); const lng = toLatLng(d.lng);
        if (lat === null || lng === null) return;
        const name = driverName(d);
        const initials = name.split(' ').filter(Boolean).map((n) => n[0]).join('').toUpperCase().slice(0, 2);
        const status = String(d.status || '').toLowerCase();
        const color = status === 'active' || status === 'on-duty' ? '#3dba7f' : '#8899bb';
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44"><circle cx="22" cy="22" r="20" fill="${color}" stroke="#fff" stroke-width="3"/><text x="22" y="27" text-anchor="middle" font-size="13" font-weight="700" font-family="Inter,sans-serif" fill="#fff">${initials}</text></svg>`;
        const marker = new gm.Marker({
          position: { lat, lng }, map: googleMapRef.current,
          icon: { url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`, scaledSize: new gm.Size(44, 44), anchor: new gm.Point(22, 22) },
          title: name, zIndex: 10, cursor: 'pointer',
        });
    ...
    (async () => {
      try {
        ...
        googleMapRef.current = new gm.Map(mapRef.current, {
          center: { lat: 32.7765, lng: -79.9311 }, zoom: 13, mapTypeId: 'roadmap', styles: MAP_STYLES,
          streetViewControl: false, mapTypeControl: false, fullscreenControl: true, zoomControl: true,
        });
        setMapReady(true);
      } catch (err) {
        if (!cancelled) { setMapError('Failed to load Google Maps. Check the API key and network connection.'); console.error('MapPage:', err); }
      }
    })();
    ...
    <div className="flex flex-col gap-4 lg:flex-row">
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex flex-wrap gap-3 px-1">
          ...
          <div className="flex items-center gap-1.5"><span className="inline-block h-4 w-4 rounded-full bg-[#3dba7f] border-2 border-white" /><span className="text-xs text-muted-foreground">Active driver</span></div>
          <div className="flex items-center gap-1.5"><span className="inline-block h-4 w-4 rounded-full bg-[#8899bb] border-2 border-white" /><span className="text-xs text-muted-foreground">Off-duty driver</span></div>
        </div>
      </div>
          ...
          <Card>
            ...
            <CardContent className="pt-0 px-4 pb-4 space-y-2 text-sm">
              ...
              {(selectedDriver.route_id || selectedDriver.routeId) && (
                <p className="text-muted-foreground">Route: {selectedDriver.route_id || selectedDriver.routeId}</p>
              )}
            </CardContent>
          </Card>
        )}
            ...
            {activeDrivers.length === 0 ? (
              ...
              <ul className="divide-y divide-border">
                {activeDrivers.map((d, i) => (
                  <li key={driverId(d, i)} className="flex items-start justify-between gap-2 px-2 py-2 hover:bg-muted/40 cursor-pointer rounded" onClick={() => setSelectedDriver(d)}>
        ...
        <Card>
          ...
          <CardContent className="pt-0 px-4 pb-4">
            {Object.keys(stopStatusColors).map((s) => {
              const count = stops.filter((stop) => String(stop.status || '').toLowerCase().replace(/[\s_]+/g, '-') === s).length;
              return (
            ...
            {stops.length === 0 && <p className="text-xs text-muted-foreground">No stop data.</p>}
          </CardContent>
        </Card>
      </div>
    </div>
