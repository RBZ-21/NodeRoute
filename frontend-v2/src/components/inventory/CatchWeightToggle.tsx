...
export function CatchWeightToggle({ item, onToggled }: { item: InventoryItem; onToggled: (updated: { item_number: string; is_catch_weight: boolean }) => void }) {
  ...
  return (
    <button onClick={toggle} disabled={saving}
      ...
      aria-label={item.is_catch_weight ? 'Disable catch weight' : 'Enable catch weight'}
      ...
      <span className="sr-only">{item.is_catch_weight ? 'Catch weight on' : 'Catch weight off'}</span>
    </button>
  );
}
